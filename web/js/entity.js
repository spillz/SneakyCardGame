class Entity {
    constructor(tile, sprite) {
        ...
        this.bounding_box = new Rect([0.25, 0.25, 0.5, 0.75]);
        this.hit_box = new Rect(this.bounding_box);
        this.facing = 1;
        this.falling = false;
        this.canFall = true;
    ...
    rotatedBoundingBox(rotatePt=null, angle = null, flipped = null, center=null, size=null) {
        ...
        // center -- coordinates of the center of the bounding box to be rotated (defaults to center of this.bounding_box)
        ...
        if(center==null) {
            center = [this.bounding_box[0]+this.bounding_box[2]/2, this.bounding_box[1]+this.bounding_box[3]/2];
        }
        if(size==null) {
            size = this.bounding_box.slice(2,4);
        }
    ...
    bounds(pos = null, bb=null) {
        ...
        if (bb == null) {
            bb = this.bounding_box!=null? this.bounding_box: this.bounding_boxes[0];
        }
    ...
    hit_bounds(pos = null) {
        if (pos == null)
            pos = this.pos;
        return new Rect([pos[0] + this.hit_box[0], pos[1] + this.hit_box[1], this.hit_box[2], this.hit_box[3]]);
    }
    ...
    draw_bounds() {
        if(this.bounding_box !=null) {
            let r = this.bounds(this.pos, this.bounding_box);
            ...
            for(let b of this.bounding_boxes) {
    ...
    entity_interact(entity, action) {
    }

    entity_collide(entity) {
    }
...
class DeadPlayer extends Entity {
    constructor(player) {
        super(game.tiles.closest_tile(player.bounds()), new Vec2([1, player.sprite]));
        this.elapsed = 0;
        this.timer = 3000;
    }
    update(millis) {
...
class MonsterBoom extends Entity {
    ...
    update(millis) {
        ...
        if (this.elapsed >= this.timer || player != null && !player.dead && player.oldControlStates['use'] && !player.controlStates['use']) {
            ...
            let t = game.tiles.closest_tile(this.monster.bounds());
            let t0 = game.tiles.left_of(t);
            let t1 = game.tiles.right_of(t);
            ...
            for (let m of game.monsters)
                if (m.hit_bounds().collide(bigRect))
                    m.hit(this.damage);
            for (let player of game.activePlayers)
                if (player.hit_bounds().collide(bigRect))
...
class Shot extends Entity {
    constructor(player, shotDamage = 1, angle = 0, facing = 1, speedRatio=1.0, pos=null) {
        ...
        this.canFall = false;
        this.bounding_box = new Rect([7 / 16, 7 / 16, 2 / 16, 2 / 16]);
    ...
    update(millis) {
        ...
        if (this.vel.x != velx || this.vel.y != vely) {
            ...
            for (let m of monsters) {
                if (!m.dead && this.bounds().collide(m.hit_bounds())) {
...
class LiveGrenade extends Entity {
    constructor(player, damage = 3, radius = 2, facing = 1, base_vel = 1.0/80) {
        ...
        this.bounding_box = new Rect([0.4, 0.4, 0.2, 0.2])
        this.player = player;
        this.pos = player.pos.add([facing * player.bounding_box.w * 0.75, 0]);
        let t = game.tiles.closest_tile(this.bounds());
    ...
    update(millis) {
        ...
        if (this.elapsed >= this.timer || player != null && !player.dead && player.oldControlStates['use'] && !player.controlStates['use']) {
            ...
            let t0 = game.tiles.closest_tile(this.bounds());
...
class ShotFrags extends Entity {
    ...
    constructor(player, shotDamage = 3, shotRange = 2, angle = 0, facing = 1) {
        ...
        this.bounding_boxes = [this.rotatedBoundingBox([-0.5,1], this.angle, this.facing<0, [0.5,1],[1,1]),
    ...
    update(millis) {
        if (this.elapsed == 0) {
            ...
            for (let m of monsters) {
                for(let b of this.bounding_boxes) {
                    if(!m.dead && this.bounds(this.pos, b).collide(m.hit_bounds())) {
                        m.hit(this.shotDamage);
                        break;
                    }
                }
...
class LiveRocket extends Entity {
    ...
    update(millis) {
        ...
        if (this.pos.x <= -4 || this.pos.x >= 4 + game.tiles.dimW) {
            ...
            let monsters = monsters_with_other_players(this.player);
            for (let m of monsters) {
                if (m.hit_bounds().collide(this.bounds())) {
                    collision = true;
                    break;
                }
            }
...
class LiveDrone extends Entity {
    constructor(player, invItem) {
        ...
        this.pos = player.pos.add([player.facing * player.bounding_box.w * 0.75, 0]);
...
class SaberStrike extends Entity {
    constructor(pos, facing, angle = 0) {
        ...
        this.elapsed = 0;
        this.lifeTime = 100;
//        this.bounding_box = new Rect([0,0.25,0.75,0.5])
        this.bounding_box = this.rotatedBoundingBox([-0.25, 0.5]);
    }
    update(millis) {
        this.elapsed += millis;
        // drone dies if
...
class GrappleLine extends Entity { // TODO: This is unfinished copy
    constructor(player, invItem, angle, vel, power) {
        super(null, -1);
        this.player = player;
        ...
        this.bounding_box = new Rect([0.45, 0.45, 0.1, 0.1])
        this.timer = 2000;
        this.elapsed = 0;
...
class MovingPlatform extends Entity {
    constructor(terminal1, terminal2) {
        ...
        this.bounding_box = new Rect([0,0,1,3.0/16]);
    }
    update(millis) {
...
class TrapBlade extends Entity {
    constructor(platform) {
        ...
        this.facing = 1// 2*(terminal1.pos.x < terminal2.pos.x) - 1;
        this.bounding_box = new Rect([0.125, 0.25, 0.75, 0.75]);
        this.elapsed = 0;
    }
    update(millis) {
...
class ActiveDrill extends Entity {
    ...
    setTarget() {
        let t=game.tiles.closest_tile(this.bounds());
        this.target = null;
        let candidates = shuffle([[game.tiles.below(t),180], [game.tiles.left_of(t),270], [game.tiles.right_of(t),90]]);
