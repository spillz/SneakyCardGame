...
class Player extends Entity{
    constructor(playerId=-1, tile=null){
        super(tile, playerId);
        this.isPlayer = true;
        this.bounding_box = new Rect([0.25,0.5,0.5,0.5]);
    ...
    hit(damage, knockback_scale=0) {
        ...
        if(!this.dead && !this.escaped && this.immunityTimer.finished()) {
            for(let i of this.hookHitModifier) {
                [damage, knockback_scale] = i.hitModifier(damage, knockback_scale);
            }
            super.hit(damage, knockback_scale);
    ...
    entityInteract() {
        for(let k=0;k<game.items.length;k++) {
            if(this.bounds().collide(game.items[k].bounds())) {
                game.items[k].entity_collide(this);
                if(!this.aiming) {
                    if(this.controlStates['up']) {
                        game.items[k].entity_interact(this, 'up');
                    }
                    if(this.controlStates['down']) {
                        game.items[k].entity_interact(this, 'down');
                    }
                }
            }
        }
    ...
    tileInteract(millis) {
        if(this.controlStates['up']) {
            ...
            if(!(t instanceof Void)) {
                t.entity_interact(this,'up');
            }
        }
        if(this.controlStates['down']) {
            let t=game.tiles.closest_tile(this.bounds());
            if(!(t instanceof Void)) {
                t.entity_interact(this,'down');
                ...
                {
                    tdown.entity_interact(this,'down');
                }
            }
